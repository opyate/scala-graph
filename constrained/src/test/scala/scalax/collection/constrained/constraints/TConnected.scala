...
class TConnected [CC[N,E[X] <: EdgeLikeIn[X]] <: Graph[N,E] with GraphLike[N,E,CC[N,E]]]
  ...
  with    ShouldMatchers
{
  ...
  def test_create {
    val g1 = Graph(Connected)(1~>2, 2~>3)
    g1 should have size (5)
    val g2 = Graph(Connected)(1~>2, 3~>4)
